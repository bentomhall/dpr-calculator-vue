...
export class Ranger extends ClassEntity {
    ...
    constructor(options: ClassOptions | null, provider: AccuracyProvider, mode: AccuracyMode) {
        ...
        this.options = {
            ...
            hasArcheryFS: options?.toggles?.get('archery') ?? false,
    ...
    public calculate(type: string, level: number): DamageOutput {
        ...
        return attackProvider.weaponAttacks(level, attacks, attackModifier, new AttackDamageOptions(this.options.weaponDie, modifier, hm, 2*hm, 0, true, false))
    }
